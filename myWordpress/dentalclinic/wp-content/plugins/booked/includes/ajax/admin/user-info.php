...
echo '<div class="booked-scrollable">';
	...
	if (!$_POST['user_id'] && isset($_POST['appt_id'])):
		...
		echo '<p class="booked-modal-title">'.__('Contact Information','booked').'</p>';
		echo '<p class="booked-modal-list-item"><strong class="booked-left-title">'.__('Name','booked').':</strong> '.$guest_name.'</p>';
		if ($guest_email) : echo '<p class="booked-modal-list-item"><strong class="booked-left-title">'.__('Email','booked').':</strong> <a href="mailto:'.$guest_email.'">'.$guest_email.'</a></p>'; endif;
		...
		echo '<p class="booked-modal-title">'.__('Contact Information','booked').'</p>';
		echo '<p class="booked-modal-list-item"><strong class="booked-left-title">'.__('Name','booked').':</strong> '.$display_name.'</p>';
		if ($email) : echo '<p class="booked-modal-list-item"><strong class="booked-left-title">'.__('Email','booked').':</strong> <a href="mailto:'.$email.'">'.$email.'</a></p>'; endif;
		if ($phone) : echo '<p class="booked-modal-list-item"><strong class="booked-left-title">'.__('Phone','booked').':</strong> <a href="tel:'.preg_replace('/[^0-9+]/', '', $phone).'">'.$phone.'</a></p>'; endif;
	...
	if (isset($_POST['appt_id'])):
		...
		echo '<p class="booked-modal-title bm-title-bordered">'.__('Appointment Information','booked').'</p>';
		do_action('booked_before_appointment_information_admin');
		echo '<p class="booked-modal-list-item"><strong class="booked-left-title">'.__('Date','booked').':</strong> '.$day_name.', '.$date_display.'</p>';
