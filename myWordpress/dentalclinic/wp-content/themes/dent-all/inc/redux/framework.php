...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once REDUX_PATH . '/inc/class.redux_instances.php';
        require_once REDUX_PATH . '/inc/lib.redux_instances.php';
    }
...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux CDN class
        require_once REDUX_PATH . '/inc/class.redux_cdn.php';

        // Redux API class  :)
        require_once REDUX_PATH . '/inc/class.redux_api.php';

        // General helper functions
        require_once REDUX_PATH . '/inc/class.redux_helpers.php';

        // General functions
        require_once REDUX_PATH . '/inc/class.redux_functions.php';
        require_once REDUX_PATH . '/inc/class.p.php';

        require_once REDUX_PATH . '/inc/class.redux_filesystem.php';

        require_once REDUX_PATH . '/inc/class.redux_admin_notices.php';

        // ThemeCheck checks
        require_once REDUX_PATH . '/inc/themecheck/class.redux_themecheck.php';

        // Welcome
        //require_once REDUX_PATH . '/inc/welcome/welcome.php';
        ...
        class ReduxFramework {
            ...
            public static function init() {
                $dir = Redux_Helpers::cleanFilePath( REDUX_PATH );

                // Windows-proof constants: replace backward by forward slashes. Thanks to: @peterbouwmeester
                self::$_dir           = trailingslashit( $dir );
                self::$wp_content_url = trailingslashit( Redux_Helpers::cleanFilePath( ( is_ssl() ? WP_CONTENT_URL : WP_CONTENT_URL ) ) );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'dent-all' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'dent-all' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'dent-all' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'dent-all' );
                }
...
// get_instance()
            ...
            private function _tracking() {
                if ( file_exists( REDUX_PATH . '/inc/tracking.php' ) ) {
                    require_once REDUX_PATH . '/inc/tracking.php';
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'dent-all' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'dent-all' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'dent-all' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'dent-all' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'dent-all' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'dent-all' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'dent-all' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
// _register_settings()
            ...
            private function _register_extensions() {
                $path    = REDUX_PATH . '/inc/extensions/';
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => __( 'Invalid security credential.  Please reload the page and try again.', 'dent-all' ),
                        'action' => ''
                    ) );
                ...
                if (!current_user_can ( $this->args['page_permissions'] )) {
                    echo json_encode( array(
                        'status' => __( 'Invalid user capability.  Please reload the page and try again.', 'dent-all' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'dent-all' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
