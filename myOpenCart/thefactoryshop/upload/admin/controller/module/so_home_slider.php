...
class ControllerModuleSohomeslider extends Controller {
	...
	// Get data
		$model = $this->model_module_so_home_slider; 
		$model->checkInstall();
			
	// Delete Module
		if( isset($this->request->get['slide_id']) && isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			$this->model_module_so_home_slider->deleteSlide( $this->request->get['slide_id'] );
			$this->response->redirect($this->url->link('module/so_home_slider', 'token=' . $this->session->data['token'].'&tab=slide&module_id='.$this->request->get['module_id'], 'SSL'));
			...
			$this->model_module_so_home_slider->deleteAllSlide($this->request->get['module_id']);
			$this->response->redirect($this->url->link('module/so_home_slider', 'token=' . $this->session->data['token'], 'SSL'));
		}
	// Get Module Id New
		$moduleid_new= $this->model_module_so_home_slider->getModuleId(); // Get module id
		$module_id = '';	
		$slideid_new= $this->model_module_so_home_slider->getSlideId(); // Get slide id
		$slide_id = '';
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($tab_module == 'add-slide' || $tab == 'slide' && $tab_module != 'add-module')
			{
				if (!isset($this->request->get['slide_id'])) {
					...
					$this->model_module_so_home_slider->addSlide($this->request->post);
				} else {
					$slide_id = $this->request->get['slide_id'];
					$this->request->post['moduleid'] = $this->request->get['module_id'];
					
					$this->model_module_so_home_slider->editSlide($this->request->get['slide_id'], $this->request->post);
				}
				
				if($action == "save_edit") {
					$this->response->redirect($this->url->link('module/so_home_slider', 'module_id='.$this->request->get['module_id'].'&tab=slide&slide_id='.$slide_id.'&token=' . $this->session->data['token'], 'SSL'));
				...
				if($action == "save_edit") {
					$this->response->redirect($this->url->link('module/so_home_slider', 'module_id='.$module_id.'&token=' . $this->session->data['token'], 'SSL'));
				}elseif($action == "save_new"){
					$this->response->redirect($this->url->link('module/so_home_slider', 'token=' . $this->session->data['token'], 'SSL'));
				}else{
					$this->response->redirect($this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL'));
		...
		if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST') || ($this->request->server['REQUEST_METHOD'] == 'POST' && !$this->validate() && isset($this->request->get['module_id'])) || ($this->request->server['REQUEST_METHOD'] == 'POST' && !$this->validate() && isset($this->request->get['slide_id']))) {
			...
			$data['action'] 				= $this->url->link('module/so_home_slider', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
			
			if(isset($this->request->get['slide_id'])){
				$data['action'] = $this->url->link('module/so_home_slider', 'token=' . $this->session->data['token'] . '&tab=slide&module_id='.$this->request->get['module_id'].'&slide_id=' . $this->request->get['slide_id'], 'SSL');
				
			}elseif(isset($this->request->get['tab']) && $this->request->get['tab'] == "slide")
			{
				$data['action'] = $this->url->link('module/so_home_slider', 'token=' . $this->session->data['token'] . '&tab=slide&module_id='.$this->request->get['module_id'], 'SSL');
			}
			...
			if(isset($this->request->get['slide_id'])){
				$data['slide_description'] = $this->model_module_so_home_slider->getSliderById($this->request->get['slide_id']);
				$language = count($data['slide_description']);
				
				if($data['slide_description'][1]['image'] != "")
				{
					if (is_file(DIR_IMAGE . $data['slide_description'][1]['image'])) {
						$data['slide_description']['thumb'] = $this->model_tool_image->resize($data['slide_description'][1]['image'], 100, 100);
						
					} else {
						$data['slide_description']['thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
					}
				}
				$data['slide_description']['image'] = $data['slide_description'][1]['image'];
				$data['slide_description']['url'] = $data['slide_description'][1]['url'];
				$data['slide_description']['status'] = $data['slide_description'][1]['status'];
			}
			...
			$data['selectedid'] 	= 0;
			$data['action'] 		= $this->url->link('module/so_home_slider', 'token=' . $this->session->data['token'], 'SSL');
		...
		$data['link'] = $this->url->link('module/so_home_slider', 'token=' . $this->session->data['token'] . '', 'SSL');
		//--------------------------------Load Data -------------------------------------------
		...
		$this->response->setOutput($this->load->view('module/so_home_slider.tpl', $data));
	}
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/so_home_slider', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/so_home_slider', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		return $this->data['breadcrumbs'];
	}
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/so_home_slider')) {
	...
	public function sortposition() {
		...
		if (isset($this->request->get['sortarray'])) {
			$result = $this->model_module_so_home_slider->updatePositionSlide($this->request->get['sortarray']);
		}
	}
}