	...
	$win.on('load', function() {
				...
				if (startTime && endTime && count){
					...
					if (appt_add_confirm == true){
						booked_js_vars.ajaxRequests.push = $.bookedAjaxQueue({
							...
							url 	: booked_js_vars.ajax_url,
							data	: {
								'action'     			: 'booked_admin_add_custom_timeslot',
								'startTime'     		: startTime,
								'endTime'     			: endTime,
								'count'     			: count,
				...
				if (startTime && endTime && interval && count){
					...
					if (appt_add_confirm == true){
						...
						booked_js_vars.ajaxRequests.push = $.bookedAjaxQueue({
							...
							url 	: booked_js_vars.ajax_url,
							data	: {
								'action'     			: 'booked_admin_add_custom_timeslots',
								'startTime'     		: startTime,
								'endTime'     			: endTime,
								'interval'     			: interval,
		...
		function loadCustomTimeSlots(timeslotsBlock,json_array){
			...
			$.ajax({
				url		: booked_js_vars.ajax_url,
				type	: 'post',
				data	: {'action':'booked_admin_custom_timeslots_list','json_array':json_array},
				success	: function(html){
					...
					if (startTime && endTime && interval && count){
						...
						if (appt_add_confirm == true){
							...
							booked_js_vars.ajaxRequests.push = $.bookedAjaxQueue({
								...
								url 	: booked_js_vars.ajax_url,
								data	: {
									'action'     			: 'booked_admin_add_timeslots',
									'day'     				: day,
					...
					if (startTime && endTime && count){
						...
						if (appt_add_confirm == true){
							...
							booked_js_vars.ajaxRequests.push = $.bookedAjaxQueue({
								...
								url 	: booked_js_vars.ajax_url,
								data	: {
									'action'     			: 'booked_admin_add_timeslot',
									'day'     				: day,
									'calendar_id'			: calendar_id,
									'startTime'     		: startTime,
									'endTime'     			: endTime,
		...
		$('.booked-admin-calendar-wrap').on('click', 'tr.entryBlock button.new-appt', function(e) {
			...
			booked_load_calendar_date_booking_options = {'action':'booked_admin_new_appointment_form','date':date,'timeslot':timeslot,'calendar_id':calendar_id};
