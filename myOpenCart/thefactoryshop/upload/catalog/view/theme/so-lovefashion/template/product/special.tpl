...
/* PAGE PRODUCT */
		...
		if (isset($soconfig_general[$store_id]['lazy_loader'])) 	{$soconfig_general['lazy_loader'] = $soconfig_general[$store_id]['lazy_loader'];}
...
/*VARIABLES*/
	...
	$lazyload_status = $soconfig_general["lazy_loader"];
      ...
      <?php if ($products) { ?>
			...
			<div class="product-filter filters-panel">
				...
				<?php if (!empty($pagination)){?>
					<div class="box-pagination text-right"><?php echo $pagination; ?></div>
				<?php }?>
			</div>
			<!-- //end Filters -->
	
			<!--Changed Listings-->
		<div class="products-list page-search row <?php echo ($soconfig_pages['product_catalog_mode'] =='1') ? 'list' : 'grid'?>">
        <?php 
		
		$device_res  ='';
		foreach ($devices as $subfix => $device) {
			$device_res .=  'col-'.$subfix.'-'.ceil(12/$soconfig_pages["product_catalog_refine_col_".$subfix] ).' ';
		}
			...
			<div class="product-item-container">
				<div class="left-block">
					<div class="product-image-container <?php echo $lazyload_text?> <?php if($secondimg =='2' && isset($product['thumb2'])){ echo "second_img";} ?> ">
						<div class="image">
							<img data-src="<?php echo $product['thumb']; ?>" src="<?php echo lazyload($product['thumb']); ?>"  title="<?php echo $product['name']; ?>" class="img_1 img-responsive" />
						...
						<?php if (!isset($quick_status) || $quick_status != 0) : ?>
							<!--full quick view block-->
								...
								<a class="quickview iframe-link visible-lg" data-fancybox-type="iframe"  href="<?php echo $our_url->link('product/quickview','product_id='.$product['product_id']);?>"></a>
				...
				<div class="right-block">
					<div class="caption">
					<?php $title_product= substr($product['name'], 0 , 35 );?>
					<h4><a href="<?php echo $product['href']; ?>" title="<?php echo $product['name']; ?>"><?php echo $title_product; ?></a></h4>		
