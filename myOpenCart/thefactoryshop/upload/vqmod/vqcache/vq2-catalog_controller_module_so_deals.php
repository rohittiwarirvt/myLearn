...
class ControllerModuleSodeals extends Controller {
	public function index($setting) {
		static $module = 1;
			...
			$data['text_mins'] = $this->language->get('text_mins');
			$data['text_sec'] = $this->language->get('text_sec');
			$data['text_secs'] = $this->language->get('text_secs');
			$data['quick_view_text'] = $this->language->get('quick_view_text');
	...
	public function getProducts($category_id_list,$setting)
	{
		...
		if ($cat['count'] > 0) 
		{
			...
			{	
				...
				$datetimeCreate = new DateTime($product_info['date_added']);
