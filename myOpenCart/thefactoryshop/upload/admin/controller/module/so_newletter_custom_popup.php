...
class ControllerModuleSonewlettercustompopup extends Controller {
	...
	public function index() {
		...
		$this->model_module_so_newletter_custom_popup->createNewsletter();
		...
		if( isset($this->request->get['module_id']) && isset($this->request->get['delete']) ){
			...
			$this->response->redirect($this->url->link('module/so_newletter_custom_popup', 'token=' . $this->session->data['token'], 'SSL'));
		}
		// Get Module Id new
		$moduleid_new= $this->model_module_so_newletter_custom_popup->getModuleId();
		$module_id = '';
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if($action == "save_edit") {
				$this->response->redirect($this->url->link('module/so_newletter_custom_popup', 'module_id='.$module_id.'&token=' . $this->session->data['token'], 'SSL'));
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('module/so_newletter_custom_popup', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('module/so_newletter_custom_popup', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$data['link'] = $this->url->link('module/so_newletter_custom_popup', 'token=' . $this->session->data['token'] . '', 'SSL');
		//--------------------------------Load Data -------------------------------------------
		...
		$this->response->setOutput($this->load->view('module/so_newletter_custom_popup/so_newletter_custom_popup.tpl', $data));
	}
	public function _breadcrumbs(){
		...
		if (!isset($this->request->get['module_id'])) {
			$this->data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/so_newletter_custom_popup', 'token=' . $this->session->data['token'], 'SSL')
			);
		} else {
			$this->data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/so_newletter_custom_popup', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		return $this->data['breadcrumbs'];
	}
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'module/so_newletter_custom_popup')) {
	...
	public function history()
	{
		...
		$data['objlang'] = $this->language;
		$this->load->model('module/so_newletter_custom_popup');
		$data['newletter_email'] = $this->model_module_so_newletter_custom_popup->getListemail();
		$this->response->setOutput($this->load->view('module/so_newletter_custom_popup/so_newletter_subscribers.tpl', $data));
	}
	public function approve_selected()
	{
		$this->load->model('module/so_newletter_custom_popup');
		$json = array();
		$json['success'] = $this->model_module_so_newletter_custom_popup->approve_status($_GET['subscribe_id']);
	...
	public function approve_all_selected()
	{
		if(!$this->request->post){
			...
			$this->load->model('module/so_newletter_custom_popup');
			$json = array();
			$check = $this->model_module_so_newletter_custom_popup->approve_all_selected($this->request->post);
			if (isset($check)) {
	...
	public function approve_all_not_approved()
	{
		$this->load->model('module/so_newletter_custom_popup');
		$json = array();
		$json['success'] = $this->model_module_so_newletter_custom_popup->approve_all_not_approved();
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	...
	public function delete_selected()
	{
		$this->load->model('module/so_newletter_custom_popup');
		$json = array();
		$json['success'] = $this->model_module_so_newletter_custom_popup->delete_item($_GET['subscribe_id']);
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	...
	public function delete_all()
	{
		$this->load->model('module/so_newletter_custom_popup');
		$json = array();
		$json['success'] = $this->model_module_so_newletter_custom_popup->delete_all();
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	...
	public function delete_all_selected()
	{
		if(!$this->request->post){
			...
			$this->load->model('module/so_newletter_custom_popup');
			$json = array();
			$check = $this->model_module_so_newletter_custom_popup->delete_all_selected($this->request->post);
			if (isset($check)) {
	...
	public function delete_all_not_approved()
	{
		$this->load->model('module/so_newletter_custom_popup');
		$json = array();
		$json['success'] = $this->model_module_so_newletter_custom_popup->delete_all_not_approved();
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	...
	public function revert_yet_send()
	{
		$this->load->model('module/so_newletter_custom_popup');
		$json = array();
		$json['success'] = $this->model_module_so_newletter_custom_popup->revert_yet_send();
		$this->response->addHeader('Content-Type: application/json');
		$this->response->setOutput(json_encode($json));
	...
	public function mailing_all()
	{
		...
		$emails = array();
		$results = $this->model_module_so_newletter_custom_popup->mailing_all();
		...
		try{
			if($title =='' || $content ==''){
				...
				$this->model_module_so_newletter_custom_popup->getconfirm_mail_all();
			}
		}catch (Exception $e)
		{
			$json['error'] =  $e;
	...
	public function mailing_selected()
	{
		...
		$emails = array();
		$results = $this->model_module_so_newletter_custom_popup->mailing_selected($_GET['subscribe_id']);
		foreach ($results as $result) {
			$emails[] = $result['news_email'];
		}
		try{
			if($title =='' || $content ==''){
				...
				$this->sendmail($emails, $title, $content);
				$json['success'] = "Send mail Successfull";
				$this->model_module_so_newletter_custom_popup->getconfirm_mailing_selected($_GET['subscribe_id']);
			}
		}catch (Exception $e)
		{
			$json['error'] =  $e;
	...
	public function mailing_all_selected()
	{
		if(!$this->request->post){
			...
			$emails = array();
			$results = $this->model_module_so_newletter_custom_popup->mailing_all_selected($this->request->post);
			foreach ($results as $result) {
				foreach ($result as $r) {
					$emails[] = $r['news_email'];
				}
			...
			try {
				if($title =='' || $content ==''){
					...
					$this->sendmail($emails, $title, $content);
					$json['success'] = "Send mail Successfull";
					$this->model_module_so_newletter_custom_popup->getconfirm_mail_all_selected($this->request->post);
				}
			} catch (Exception $e) {
				$json['error'] = $e;
			}
	...
	public function mailing_all_not_notified()
	{
		...
		$emails = array();
		$results = $this->model_module_so_newletter_custom_popup->mailing_all_not_notified();
		foreach ($results as $result) {
			$emails[] = $result['news_email'];
		}
		try{
			if($title =='' || $content ==''){
				...
				if (!$emails) {
					...
					$this->sendmail($emails, $title, $content);
					$json['success'] = "Send mail Successfull";
					$this->model_module_so_newletter_custom_popup->getconfirm_mail_all_not_notified();
				}
			}
		}catch (Exception $e)
		{
	...
	public function mailing_all_approved()
	{
		...
		$emails = array();
		$results = $this->model_module_so_newletter_custom_popup->mailing_all_approved();
		foreach ($results as $result) {
			$emails[] = $result['news_email'];
		}
		try{
			if($title =='' || $content ==''){
				...
				if(!$emails){
					...
					$this->sendmail($emails, $title, $content);
					$json['success'] = "Send mail Successfull";
					$this->model_module_so_newletter_custom_popup->getconfirm_mail_all_approved();
				}
			}
		}catch (Exception $e)
		{
